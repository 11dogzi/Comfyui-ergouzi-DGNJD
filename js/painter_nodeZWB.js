  ...
  makeElements() {
    ...
    panelPaintBox.innerHTML = `<div class="painter_manipulation_box" f_name="Locks" style="display:none;">
        <div class="comfy-menu-btns">
            <button id="lockMovementX" title="锁定横向位置">锁定横向位置</button>
            <button id="lockMovementY" title="锁定竖向位置">锁定竖向位置</button>
            <button id="lockScalingX" title="锁定宽度">锁定缩放宽度</button>
            <button id="lockScalingY" title="锁定高度">锁定缩放高度</button>
            <button id="lockRotation" title="锁定旋转">锁定旋转</button>
        </div>
        <div class="comfy-menu-btns">
            <button id="zpos_BringForward" title="向前调整图像顺序">向前调整</button>
            <button id="zpos_SendBackwards" title="向后调整图像顺序">向后调整</button>
            <button id="zpos_BringToFront" title="移到顶部">移到顶部</button>
    ...
    <div class="painter_drawning_box_property">
        <div class="property_textBox comfy-menu-btns" style="display:none;">
            <button id="prop_fontStyle" title="斜体" style="font-style:italic;">I</button>
            <button id="prop_fontWeight" title="加粗" style="font-weight:bold;">B</button>
            <button id="prop_underline" title="下划线" style="text-decoration: underline;">U</button>
            <div class="separator"></div>
            <select class="font_family_select"></select>
        </div>
    </div>
    <div class="painter_drawning_box">
        <div class="painter_mode_box fieldset_box comfy-menu-btns" f_name="功能">
            <button id="painter_change_mode" title="Enable selection mode">切换图层管理</button>
        ...
        <div class="painter_drawning_elements" style="display:block;">
            <div class="painter_grid_style painter_shapes_box fieldset_box comfy-menu-btns" f_name="功能选择">
                <button class="active" data-shape='Brush' title="画笔">✏️</button>
                <button data-shape='Erase' title="橡皮">❎</button>
                <button data-shape='Circle' title="圆形">⏺️</button>
                <button data-shape='Rect' title="方块">⏹️</button>
                <button data-shape='Triangle' title="三角">🔼</button>
                <button data-shape='Line' title="直线">➖</button>
                <button data-shape='Image' title="贴图">🖼️</button>
                <button data-shape='Textbox' title="文字">🔠</button>
            </div>
            <div class="painter_colors_box fieldset_box" f_name="画笔设置">
                <div class="painter_grid_style painter_colors_alpha">
                    <span>填充</span><span>透明度</span>
                    <input id="fillColor" type="color" value="#FF00FF" title="填充颜色">
                    <input id="fillColorTransparent" type="number" max="1.0" min="0" step="0.05" value="0.0" title="透明度">
                </div>
                <div class="painter_grid_style painter_colors_alpha">
                    <span>画笔</span><span>透明度</span>
                    <input id="strokeColor" type="color" value="#FFFFFF" title="画笔颜色">    
                    <input id="strokeColorTransparent" type="number" max="1.0" min="0" step="0.05" value="1.0" title="透明度">
                </div>
            </div>
            <div class="painter_stroke_box fieldset_box" f_name="画笔/橡皮 大小">
                <label for="strokeWidth"><span>画笔:</span><input id="strokeWidth" type="number" min="0" max="150" value="5" step="1" title="画笔大小"></label>
                <label for="eraseWidth"><span>橡皮:</span><input id="eraseWidth" type="number" min="0" max="150" value="5" step="1" title="橡皮大小"></label>
            </div>
            <div class="painter_grid_style painter_bg_setting fieldset_box comfy-menu-btns" f_name="画布颜色">
                <input id="bgColor" type="color" value="#000000" data-label="" title="背景色">
                <button bgImage="img_load" title="Add background image">加载图像</button>
                <button bgImage="img_reset" title="Remove background image">删除图像 <span style="color: var(--error-text);">✖</span></button>
            </div>
            <div class="painter_settings_box fieldset_box comfy-menu-btns" f_name="画布设置">      
            <button id="painter_canvas_size" title="画布设置">画布大小</button>  
          </div>
        </div>
    </div>
...
function PainterWidget(node, inputName, inputData, app) {
  ...
  node.addWidget("button", "重置画板", "clear_painer", () => {
...
app.registerExtension({
  ...
  async setup(app) {
    let PainerNode = app.graph._nodes.filter((wi) => wi.type == "PainterNodeZWB");
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "PainterNodeZWB") {
      // Get settings node
      if (painters_settings_json) {
